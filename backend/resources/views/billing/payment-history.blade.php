<x-layouts::app :title="__('Billing Payment History')">
    <div class="flex h-full w-full flex-1 flex-col gap-6 rounded-xl">
        <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
                ...
                <flux:text class="mt-1 text-sm text-zinc-600 dark:text-zinc-400">
                    {{ __('Payments, voids, refunds, and bill updates from order cancellations.') }}
                </flux:text>
                <x-app-breadcrumbs
                    class="mt-1.5"
                    :items="[
                        ...
                        ['label' => __('Billing Payment History')],
                    ]"
                />
            </div>
        </div>
        ...
        >
            <form
                method="GET"
                action="{{ route('orders.billing.payment-history.index') }}"
                class="grid gap-4 xl:grid-cols-2"
            >
                <div class="rounded-lg border border-zinc-200 p-4 dark:border-zinc-700">
                    <div class="grid gap-4 md:grid-cols-[minmax(0,1fr)_14rem] md:items-end">
                        <div class="min-w-0">
                            <label for="billing-payment-search" class="mb-1 block text-sm font-medium text-zinc-700 dark:text-zinc-300">
                                {{ __('Search') }}
                            </label>
                            <div class="relative">
                                <input
                                    id="billing-payment-search"
                                    name="search"
                                    type="text"
                                    data-preserve-focus="billing-payment-search"
                                    ...
                                    placeholder="{{ __('Invoice #, order #, name, email…') }}"
                        ...
                        <div>
                            <label for="billing-payment-actor" class="mb-1 block text-sm font-medium text-zinc-700 dark:text-zinc-300">
                                {{ __('Staff') }}
                            </label>
                            <select
                                id="billing-payment-actor"
                                name="actor_user_id"
                ...
                <div class="rounded-lg border border-zinc-200 p-4 dark:border-zinc-700">
                    <div class="grid gap-4 sm:grid-cols-2 sm:items-end">
                        <div>
                            <label for="billing-payment-from" class="mb-1 block text-sm font-medium text-zinc-700 dark:text-zinc-300">
                                {{ __('From') }}
                            </label>
                            <flux:input
                                id="billing-payment-from"
                                type="date"
                                name="date_from"
                                :label="false"
                        ...
                        <div>
                            <label for="billing-payment-to" class="mb-1 block text-sm font-medium text-zinc-700 dark:text-zinc-300">
                                {{ __('To') }}
                            </label>
                            <flux:input
                                id="billing-payment-to"
                                type="date"
                                name="date_to"
                                :label="false"
                        ...
                        <div class="sm:col-span-2">
                            <div class="flex justify-end gap-2">
                                ...
                                <flux:button variant="ghost" :href="route('orders.billing.payment-history.index')" wire:navigate>
                                    {{ __('Reset') }}
                                </flux:button>
                            </div>
                        </div>
                        ...
                        @forelse($entries as $row)
                            <tr>
                                ...
                                <td class="px-4 py-3">
                                    @if($row->bill)
                                        <a
                                            href="{{ route('orders.billing.show', $row->bill) }}"
                                ...
                                <td class="px-4 py-3 hidden sm:table-cell">
                                    @if($row->bill?->order)
                                        <a
                                            href="{{ route('orders.show', $row->bill->order) }}"
                                            class="font-mono text-sky-600 underline decoration-sky-300 underline-offset-2 hover:text-sky-800 dark:text-sky-400"
                                            wire:navigate
                                        >
